                ...
                {data.recentBookings.map((booking, index) => (
                    <div key={index} className='mt-4 flex items-center justify-between'>
                        ...
                        <div className='flex items-center gap-2 font-medium'>
                            ...
                            <span className={`px-3 py-0.5 text-xs border border-borderColor rounded-full ${booking.status === 'confirmed' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{booking.status}</span>
                        </div>
                    </div>
                ))}
            </div>
