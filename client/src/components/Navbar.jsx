...
const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 flex items-center justify-between px-4 md:px-8 lg:px-12 py-3 text-white transition-all duration-300
            ${
              scrolled
                ? "bg-white/20 backdrop-blur-md border-b border-white/10 shadow-lg"
                : "bg-transparent"
            }`}
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <motion.button
            ...
            className="relative overflow-hidden w-32 px-3 py-1.5 bg-black text-white border-none rounded-md text-sm font-medium cursor-pointer z-10 group"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
            {user ? "Logout" : "Login"}
